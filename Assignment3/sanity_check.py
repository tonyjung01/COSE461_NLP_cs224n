...
def generate_outputs(model, source, target, vocab):
    ...
    # 1f
    # Inputs
    Ybar_t = torch.load("./sanity_check_en_es_data/Ybar_t.pkl", weights_only=False)
    enc_hiddens_proj = torch.load(
        "./sanity_check_en_es_data/enc_hiddens_proj.pkl", weights_only=False
    )
    reinitialize_layers(model)
    # Run Tests
    with torch.no_grad():
    ...
    torch.save(
        dec_state_target, "./sanity_check_en_es_data/dec_state.pkl", weights_only=False
    )
    torch.save(o_t_target, "./sanity_check_en_es_data/o_t.pkl", weights_only=False)
    torch.save(e_t_target, "./sanity_check_en_es_data/e_t.pkl", weights_only=False)
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    enc_hiddens_target = torch.load(
        "./sanity_check_en_es_data/enc_hiddens.pkl", weights_only=False
    )
    dec_init_state_target = torch.load(
        "./sanity_check_en_es_data/dec_init_state.pkl", weights_only=False
    )
    ...
    assert enc_hiddens_target.shape == enc_hiddens_pred.shape, (
    ...
    assert dec_init_state_target[0].shape == dec_init_state_pred[0].shape, (
    ...
    assert dec_init_state_target[1].shape == dec_init_state_pred[1].shape, (
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    enc_hiddens = torch.load(
        "./sanity_check_en_es_data/enc_hiddens.pkl", weights_only=False
    )
    enc_masks = torch.load(
        "./sanity_check_en_es_data/enc_masks.pkl", weights_only=False
    )
    target_padded = torch.load(
        "./sanity_check_en_es_data/target_padded.pkl", weights_only=False
    )

    # Load Outputs
    combined_outputs_target = torch.load(
        "./sanity_check_en_es_data/combined_outputs.pkl", weights_only=False
    )
    ...
    assert combined_outputs_target.shape == combined_outputs_pred.shape, (
...
def question_1f_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    dec_state_target = torch.load(
        "./sanity_check_en_es_data/dec_state.pkl", weights_only=False
    )
    o_t_target = torch.load("./sanity_check_en_es_data/o_t.pkl", weights_only=False)
    e_t_target = torch.load("./sanity_check_en_es_data/e_t.pkl", weights_only=False)
    ...
    assert dec_state_target[0].shape == dec_state_pred[0].shape, (
    ...
    assert dec_state_target[1].shape == dec_state_pred[1].shape, (
